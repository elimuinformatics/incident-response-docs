...
description: This is an open-source version of Elimu Informatics's Incident Response training course. What started as an internal course to train new Incident Commanders at Elimu Informatics has since developed into training that we now deliver publicly. Learn more about incident response and the role of an Incident Commander.
---

!!!info "Incident Response Training Course (2018)"
    This is an open-source version of "Incident Response Training", our incident response training course for incident response and incident command. It started as an internal course to train new Incident Commanders and has since developed into one that we now deliver publicly. This is a snapshot of what the training looked like in 2018.

    It includes lots of introductory information on our process, and details on the Incident Commander role specifically. All the information is already available as part of our [public documentation](https://response.elimuinformatics.com), this is just a different way of presenting it that's hopefully more engaging. Feel free to use this as a base for training in your own organization.

    The text presented here is a semi-accurate transcription of how the training was usually delivered.
...
The goal of this session is to give you an understanding of how to effectively manage incidents within your organization. I'll describe the process we use at Elimu Informatics for managing critical incidents, and talk in more detail about a specific role called the "Incident Commander".

This isn't a sales pitch. The intent today is to introduce you to how we manage incidents internally at Elimu Informatics, and provide you with lots of practical information you can take away to your own organizations to either start or improve your own incident response processes.
...
Here is Elimu Informatics’s definition of an incident,
...
We also have something we call a **major incident**. This is any incident where we require a coordinated response between teams. Again, this is just our definition at Elimu Informatics, feel free to use your own.
...
We also use [severity levels](../../before/severity_levels.md) to determine how severe an incident is, and what type of response it gets. We use `P5` through `P1` for our levels, but you may use a different scheme, `P0` through `P5`, or maybe even emoji, 🔥 through 💩, etc.
...
???+ aside hide-arrow "We recommend using metrics tied to business impact."
    Metrics can be very useful, and often work best when they're tied to _business impact_. For example, a good business metric might be "number of successful transactions per second"; at Amazon it could be "number of orders per second", at Netflix it might be "stream starts per second", etc. Monitoring these important business metrics will then let you use automation to determine the severity of an incident and the type of response you use.
...
So even though we have automation at Elimu Informatics, we also have a mechanism whereby anyone can trigger our major incident response process at any time. This is very important for us. **Lowering the barrier to triggering incident response leads to a dramatic increase in the speed with which incidents are resolved**.

We don't want people to sit on something because the official alarm hasn't gone off yet. If customer support gets lots of requests very quickly, it's a good sign there's something wrong, and we need them to be able to raise the alarm. Even a new intern, or someone walking past a dashboard, should feel empowered to trigger the process. If someone walks past a graph and thinks it looks wrong, I want them to be able to trigger incident response.

You might be initially hesitant to introduce this, fearing it would lead to lots of false positives. People triggering the alarm in an abundance of caution and it not really being an incident. But in practice it's often quite the opposite, people are pretty good at policing this themselves. False alarms tend to be rare, and even when they happen they're usually warranted based on the information available at the time. Even if you do get a false positive now and then, you can use it as free practice.
...
_010. Triggering incidents via chat. [Docs Reference](../../resources/chatops.md#ir-page)_
...
So let’s talk about our process a bit more. The way we do incident response at Elimu Informatics isn’t something we invented ourselves, it is heavily based on the [Incident Command System](https://en.wikipedia.org/wiki/Incident_Command_System), usually abbreviated to ICS.
...
NIMS and ICS are the basis of the process we use at Elimu Informatics, however we have heavily modified it for our needs. Turns out that things can be streamlined a bit when human life isn't on the line.
...
When developing our process at Elimu Informatics, we looked at a few of the other systems in use around the world, and chose the bits we liked the most to add to our own.
...
Whether this works for you will depend on your organization. This is how we do it at Elimu Informatics and it works well for us, but I can imagine it not being easy to get this sort of buy-in in other organizations. I used to work in the airline industry, and I don't think this rule would fly there (Get it? Airline industry. Fly. Hello? This thing on?).
...
This one definitely wasn't said by anyone here ever. Nope. Definitely not.
...
Another one that's definitely never been mentioned on any incident response call here ever.
...
Oh look, it's another one that's never ever happened here. This is a really common problem. It's easy to start incident response calls, then spend the first 10 minutes arguing over whether it was a `P3` or a `P2`. By the time you're finished, you would be 10 minutes into a `P1` and have made no progress.

So we have a rule. We don't discuss incident severity during the call. If we can't decide between two, we always assume it's the higher severity and move on.
...
_057. We're treating this as a P1. [Docs Reference](../../training/incident_commander.md#executive-swoop-questioning-severity)_

So the IC will need to make it clear we don’t discuss, and that we’re treating it as a `P1`. It may turn out to be a `P4`, who knows, it doesn’t matter. That's a discussion for the postmortem.
...
Believe it or not, a common mistake is to page every single engineer whenever you have a `P2`. I'm not joking. It's horrible. It can work great when you only have 5 engineers, less so when you have 50.
...
A good cadence is roughly once every 20-30 minutes for internal updates. Writing the update takes away time from solving the incident, so that needs to be balanced. This cadence tends to work well.
...
A common mistake is requiring all of your Incident Commanders to be experienced engineers with deep technical knowledge of all your systems. **This is one of the bigger mistakes you can make.** Remember that IC's aren't responders, they aren't the ones actually fixing the problem, so they don't need deep technical knowledge. IC's are experts at coordinating the response, not at solving technical issues. You should be relying on your SMEs for that.

**Removing the restriction on technical knowledge tends to lead to a dramatic increase in your pool of available incident commanders**, without any negative effect on your ability to respond to incidents. You can then draw ICs from all across the organization.
...
In incidents, there can be instances where the Incident Commander starts to assume the Subject Matter Expert role and attempts to solve the problem themselves. This typically happens when an engineer is the IC, and the incident is something to do with a system they helped to build. It's very tempting to say "I know how to fix this!" and jump in and solve the problem yourself. But you cannot do that as an IC.
...
_074. Don't neglect the postmortem. [Docs Reference](../../resources/anti_patterns.md#neglecting-the-postmortem-and-followup-activities)_
...
A useful practice is something some teams call a ["Failure Friday"](https://www.youtube.com/watch?v=XKl-7hB3paY){:target="_blank"}, where you purposefully inject failure into your systems to test their resilience. Treating it like a major incident, with an incident commander and everything, lets you practice while not under the stress of a real incident.

Another fun option is a game called [Keep Talking And Nobody Explodes](https://www.keeptalkinggame.com){:target="_blank"}. Yes, that's right, video games. But this game really helps to simulate a lot of the things an incident commander has to deal with, and is a great way to get some stress free practice.
...
This was just a brief taste of the training we run at Elimu Informatics for our own Incident Commanders. We had nowhere near enough time to cover everything.

Good news though! We have published our entire incident response process online. It is an exact copy of our internal documentation only with things like phone numbers removed. It's complete free to use, and is open-sourced under an Apache 2 license so you can use it in your own organizations. [It's on GitHub](https://github.com/elimuinformatics/incident-response-docs){:target="_blank"} and we do accept pull requests if you spot any mistakes or have improvement suggestions.
